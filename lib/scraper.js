...
function parseOlivetti(html, sourceUrl) {
  ...
  const entryBlock = $("#myth")
    .filter((_, el) => $(el).find("span.lemma").length > 0)
    .first();
  const locutionsBlock = $("#myth")
    .filter(
      (_, el) => normalizeText($(el).find("h3").first().text()).toLowerCase() === "locuzioni, modi di dire, esempi"
    )
    .first();

  const lemma = normalizeText(entryBlock.find("span.lemma").first().text());
  const paradigm = normalizeText(entryBlock.find("span.paradigma").first().text());
  const grammar = normalizeText(entryBlock.find("span.grammatica").first().text());
  const definitions = compactUnique(entryBlock.find("span.italiano").map((_, el) => $(el).text()).get());

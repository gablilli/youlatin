...
function extractCellTextPreservingBreaks($, cell) {
  const html = $(cell).html() || "";
  const withLineBreaks = html.replace(/<br\s*\/?>/gi, "\n");
  const textOnly = cheerio.load(`<root>${withLineBreaks}</root>`)("root").text();
  return textOnly
    .split("\n")
      ...
      $(table)
        ...
        .each((__, row) => {
          ...
          $(row)
            .find("th, td")
            .each((___, cell) => {
              const text = extractCellTextPreservingBreaks($, cell);
              if (text) {
                cells.push(text);
              }
            });

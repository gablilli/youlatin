...
function parseOlivetti(html, sourceUrl) {
  ...
  const focusedPreview = focusBodyText(bodyText, {
    startMarkers: ["Dizionario Latino - Italiano", "Dizionario Latino Olivetti"],
    endMarkers: ["Sfoglia il dizionario", "Sfoglia il diz", "I nostri link Mappa del sito"],
  });
...
function parseNihilScio(html, sourceUrl) {
  ...
  const bodyText = normalizeText($("body").text());
  const focusedPreview = focusBodyText(bodyText, {
    startMarkers: ["Vocaboli trovati", "- Declinazione di:", "Declinazione di:"],
    endMarkers: [

...
app.get("/api/olivetti", async (req, res) => {
  try {
    const parola = safeWord(req.query.word);
    const mode = safeWord(req.query.mode || "normal");
...
app.get("/api/olivetti/declension", async (req, res) => {
  try {
    const lemma = safeWord(req.query.lemma);
    if (!lemma) {
...
app.get("/api/nihilscio", async (req, res) => {
  try {
    ...
    const lang = req.query.lang === "LA_" ? "LA_" : "IT_";
